...
#
# Configuración:
#   ROOT = $(pwd) → .config/commitizen/pyproject.toml con [tool.commitizen]
#   Resolución: $ROOT/.config/commitizen/pyproject.toml → fallback relativo → default
#
...
# Resolver config de Commitizen (regla 14: .config/commitizen/pyproject.toml)
resolve_config() {
	local cfg_rel=".config/commitizen/pyproject.toml"
	local cfg_abs="${workspace}/${cfg_rel}"
	if [[ -f "$cfg_abs" ]]; then
		echo "$cfg_abs"
	elif [[ -f "$cfg_rel" ]]; then
		echo "$cfg_rel"
...
case "$action" in
	...
	init)
		...
		echo "  1. Crea o edita .config/commitizen/pyproject.toml con [tool.commitizen]."
